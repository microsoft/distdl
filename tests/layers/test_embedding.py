...
def test_batch_norm_with_training(barrier_fence_fixture,
    ...
    dist_emb = DistributedEmbedding(P_x, *embedding_shape, _weight=weight_local).to(P_x.device)

    # Confirm shape of weight partition
    P_weight_shape = np.array(P_x_shape.copy())
    num_dim_reduce = len(embedding_shape)
    P_weight_shape[:(P_x.dim - num_dim_reduce)] = 1
    assert (dist_emb.P_weight.shape - P_weight_shape).sum() == 0
