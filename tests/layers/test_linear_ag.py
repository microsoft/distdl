...
def test_linear_adjoint_input(barrier_fence_fixture,
    ...
    print('x: ', x, x.shape, x.device, P_world.rank)

    y = layer(x)
    print('w: ', layer.weight, layer.weight.shape, layer.weight.device, P_world.rank)

    print('y: ', y, y.shape, y.device, P_world.rank)
    ...
    print('dy: ', dy, dy.shape, dy.device, P_world.rank)
