...
class DistributedFusedLayerNormZero(Module):
    r"""A distributed fused layer norm layer with FSDP.
    ...
    def __init__(self, P_x, normalized_shape, eps=1e-5, collect_state=False,
        ...
        super(DistributedFusedLayerNormZero, self).__init__()
        ...
        if self.num_reduce > 1:
            raise ValueError("FusedLayerNormZero does not support normalized_shape spanning partitioned dimensions.")
    ...
    def forward(self, input):
        ...
        # Fused layernorm from flash attention
        input = layer_norm(input, weight, bias, self.eps)
