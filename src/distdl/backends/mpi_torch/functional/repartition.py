...
class RepartitionFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, P_union, x_global_structure,
        ...
        if P_y.active:
            for (sl, sh, partner), buff in zip(P_y_to_x_overlaps, P_y_to_x_buffers):
                if buff is not None:
                    ...
                    req = P_union._comm.Irecv(xfer_buff.detach(), source=partner, tag=111)
                    requests.append(req)
                else:
        ...
        if P_x.active:
            for (sl, sh, partner), buff in zip(P_x_to_y_overlaps, P_x_to_y_buffers):
                if buff is not None:
                    ...
                    req = P_union._comm.Isend(xfer_buff.detach(), dest=partner, tag=111)
                    requests.append(req)
                else:
            ...
            for (xsl, xsh, x2ypartner) in P_x_to_y_overlaps:
                if x2ypartner == "self":
                    for (ysl, ysh, y2xpartner) in P_y_to_x_overlaps:
                        if y2xpartner == "self":
                            # TODO: check this with Philipp/Russel
                            # output[ysl] = input.detach()[xsl].contiguous() 
        ...
        if P_y.active:
            output = torch.as_tensor(output, dtype=x_global_structure.dtype, device=device)
    ...
    def backward(ctx, grad_output):
        ...
        if P_x.active:
            grad_input = torch.as_tensor(grad_input, dtype=x_global_structure.dtype, device=device)
