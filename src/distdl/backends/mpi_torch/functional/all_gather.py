...
class AllGatherFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, P_allgather,
        ...
        if P_allgather.active:
            ...
            # All-gather
            P_allgather._comm.Allgather(input.detach(), gathered_data)
    ...
    def backward(ctx, grad_output):
        ...
        if P_allgather.active:
            ...
            scattered_data = torch.zeros(*input_tensor_structure.shape, 
                dtype=input_tensor_structure.dtype, device=device)

            # Re-order input array
            grad_output_flat = torch.zeros(np.prod(grad_output.shape), dtype=output_tensor_structure.dtype, 
                device=device)
            for cart, flat in zip(*slices):
                grad_output_flat[flat] = grad_output[cart].detach().reshape(-1)
        ...
        # If we had to receive data, we need to tensorify it.
        if P_allgather.active:
            grad_input = scattered_data.detach().requires_grad_(input_tensor_structure.requires_grad)
